  ...
  if (slots.length === 0) {
    return (
      <div className="rounded-lg border border-dashed border-ink/20 bg-white/70 px-5 py-8 text-center">
        ...
        <p className="mt-2 text-sm text-sumi/75">講師の空き時間が追加されるまでお待ちください。</p>
      </div>
    );
  }
        ...
        <div className="mt-4 space-y-3">
          {selectedSlots.length > 0 ? (
            selectedSlots.map((slot) => (
              <div className="rounded-md border border-ink/10 bg-white p-3" key={slot.startsAt}>
...
function BookingSubmitButton() {
  ...
  return (
    <button
      className="mt-3 inline-flex w-full items-center justify-center gap-2 rounded-md bg-ink px-3 py-2 text-sm font-medium text-white disabled:cursor-not-allowed disabled:opacity-65"
      disabled={pending}
    >
